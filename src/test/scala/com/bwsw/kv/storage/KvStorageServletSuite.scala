  ...
  describe("a KvStorageServlet"){
    ...
    describe("(get by key)") {
      ...
      it("should return 500 Internal Server Error if request to Elasticsearch failed") {
        (processor.get(_: String, _: String)).expects("someStorage", "someKey").returning(Future(Left(InternalError(getRequestFailure.error.reason)))).once
        get("/someStorage/someKey") {
    ...
    describe("(get by keys)") {
      ...
      it("get(keys) should return 500 Internal Server Error if request to Elasticsearch failed") {
        (processor.get(_: String, _: Iterable[String])).expects("someStorage", List("key1", "key2", "key3")).returning(Future(Left(InternalError(getRequestFailure.error.reason)))).once
        post("/someStorage/", "[\"key1\",\"key2\",\"key3\"]") {
          status should equal(500)
        }
      }
    ...
    describe("(set the key/value)") {
      ...
      it("set(key) should return 500 Internal Server Error if request to Elasticsearch failed") {
        (processor.set(_: String, _: String, _: String)).expects("someStorage", "someKey", "someValue").returning(Future(Left(InternalError(getRequestFailure.error.reason)))).once
        put("/someStorage/someKey", "\"someValue\"") {
          status should equal(500)
        }
      }
    ...
    describe("(set key/value pairs)") {
      it("should set multiple values by keys into storage") {
        (processor.set(_: String, _: Map[String, String])).expects("someStorage", Map(
      ...
      it("set(kvs) should return 500 Internal Server Error if request to Elasticsearch failed") {
        (processor.set(_: String, _: Map[String, String])).expects("someStorage", Map(
          "key1" -> "value1", "key2" -> "value2", "key3" -> "value3")).returning(Future(Left(InternalError(getRequestFailure.error.reason)))).once
        put("/someStorage/set", "{\"key1\":\"value1\",\"key2\":\"value2\",\"key3\":\"value3\"}") {
          status should equal(500)
        }
      }
    ...
    describe("(delete by the key)") {
      ...
      it("delete(key) should return 500 Internal Server Error if request to Elasticsearch failed") {
        (processor.delete(_: String, _: String)).expects("someStorage", "someKey").returning(Future(Left(InternalError(getRequestFailure.error.reason)))).once
        delete("/someStorage/someKey") {
          status should equal(500)
        }
      }
    ...
    describe("(delete by keys)") {
      ...
      it("delete(keys) should return 500 Internal Server Error if request to Elasticsearch failed") {
        (processor.delete(_: String, _: Iterable[String])).expects("someStorage", List("key1", "key2", "key3")).returning(Future(Left(InternalError(getRequestFailure.error.reason)))).once
        put("/someStorage/delete", "[\"key1\",\"key2\",\"key3\"]") {
          status should equal(500)
        }
      }
    ...
    describe("(list)") {
      ...
      it("list should return 500 Internal Server Error if any request to Elasticsearch failed") {
        (processor.list(_: String)).expects("someStorage").returning(Future(Left(InternalError(getRequestFailure.error.reason)))).once
        get("/someStorage/list") {
          status should equal(500)
        }
      }
    ...
    describe("(clear)") {
      ...
      it("clear should return 500 Internal Server Error if request to Elasticsearch failed") {
        (processor.clear(_: String)).expects("someStorage").returning(Future(Left(InternalError(getRequestFailure.error.reason)))).once
        put("/someStorage/clear") {
          status should equal(500)
        }
      }
